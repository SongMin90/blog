...
public class CommentServiceImpl implements ICommentService {
    ...
    public void insertComment(CommentVo comments) {
        List<CommentVo> commentVos = commentDao.selectByFrom(comments.getFrom());
        CommentVo commentVo = null;
        if(commentVos != null && commentVos.size() > 0) {
            commentVo = commentVos.get(commentVos.size() - 1);
        }
        if (null == comments) {
        ...
        if (StringUtils.isBlank(comments.getAuthor())) {
            if(commentVo != null) {
                ...
                String author;
                // 查询当前文章评论条数
                int count = commentDao.countByCid(comments.getCid());
                switch (count) {
                    case 0:
                        author = "沙发";
                        break;
        ...
        if(StringUtils.isBlank(comments.getUrl()) && commentVo != null) {
        ...
        if (comments.getContent().length() < 1 || comments.getContent().length() > 2000) {
            throw new TipException("评论字数在1-2000个字符");
        }
